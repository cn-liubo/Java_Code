 ...
 * 前端资源：/jsp/校验Session，不通过重定向到登陆页面
 *         /js/,/static/,/view/全部都不校验
 */
@WebFilter("/*")
public class LoginFilter implements Filter {
    ...
    public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse, FilterChain filterChain) throws IOException, ServletException {
        ...
        String servletPath = request.getServletPath();//获取当前请求的服务路径
        //不需要登录允许访问，往下执行继续调用
        if(servletPath.startsWith("/js/") || servletPath.startsWith("/static/") ||
            ...
            if (session == null || session.getAttribute("user") == null) {
                if (servletPath.startsWith("/jsp/")) {
                    //前端重定向到登录页面
                    response.sendRedirect(basePath(request)+"/view/login.html");
                } else {
                    //后端返回401状态码
